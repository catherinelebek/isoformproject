...
isoforms.full <- read.delim("~/Documents/Semester3/Project/Results/resultsanalysis/topisoforms.csv", 
                            header = T, sep = ",")

isoforms.full$X <- sub("\\..*","",isoforms.full$X)
head(isoforms.full)
gL <- c(t(isoforms.full$PC3))
names(gL) <- isoforms.full$X

head(gL)

# return transcripts with a loading in top 10% of transcripts
...
#                   elimFisher = resultsFisher.elim, orderBy = "classicKS", ranksOf = "classicFisher", topNodes = 1000)

allRes <- GenTable(GOdata, classicKS=resultsKS.classic, weightKS = resultsKS.weight,
                   elimKS = resultsKS.elim, orderBy = "weightKS",  ranksOf = "classicKS",
                   topNodes = 800)
...
showSigOfNodes(GOdata, score(resultFisher.classic), firstSigNodes = 10, useInfo = "all")
