...
# read in batch corrected isoform count data
...
# read in metadata in order to annotate patients by responder-type
...
# create a dataframe of samples for annotating PCA plot and running DESeq2
...
# only one of the 3 below sections should be run based on the subset of samples being used for DEA 
# do not run any of these 3 sections if want to run DEA on full set of patients
# total RNA library
# mRNA libraries (stranded and unstranded)
# DFKZ & Rabadan samples
...
# once created dataset of subset of samples for DEA, then run DEA using DESeq2
# create DESeq2 dataset using the chosen subset of samples for the countData option
...
# results comparison
# import results from all 4 DEAs
...
# rename columns to identify results by subset of samples used for DEA
...
# set threshold for colouring points in volcano plot
...
# volcano plot for DFKZ & Rabadan samples DEA
...
# plot to show overlap in results
...
# volcano plot for total RNA samples DEA
